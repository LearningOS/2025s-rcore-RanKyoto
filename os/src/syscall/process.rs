...
pub fn sys_get_time(ts: *mut TimeVal, _tz: usize) -> isize {
    ...
    let token = current_user_token();
    let us = get_time_us();
    let time_value = TimeVal{sec:us / 1_000_000,usec:us % 1_000_000};
    let time_value_bytes = unsafe {
        core::slice::from_raw_parts(
            (&time_value as *const TimeVal) as *const u8,
            core::mem::size_of::<TimeVal>(),
        )
    };
    let v = translated_byte_buffer(token, ts as *mut u8, core::mem::size_of::<TimeVal>());
    let flatten_dest_iter = v.into_iter().flatten();
    for (dest_byte, src_byte) in flatten_dest_iter.zip(time_value_bytes.iter()) {
        *dest_byte = *src_byte;
    }
    0
}
